...
Use a rough timeline early to test a short shot's budget for walking, camera turns, settling, dialogue and pauses. A temporary animatic plus a correctly spoken audio sample can reveal an impossible eight-second plan before paid generation. Later, replace placeholders with the accepted footage and preserve the agreed continuity.

## Repair boundaries

For a pronunciation error, use a correctly spoken replacement take or a verified synthesized sample within authorization; changing captions does not repair speech. Match timing, voice, room tone/ambience and levels. Off-camera speech is easier to replace; visible speech also requires a convincing lip-sync solution and review. Do not stretch a wrong pronunciation into an assumed correction.

Use cuts, sound bridges, ordinary masks or tracked replacements only where the source supports them and the brief permits them. Do not hide an unintended jump with an unapproved cut in a requested continuous shot. A small sign on a stable surface may be repairable; constantly warping architecture, teleporting stalls and broken viewpoint usually need a rebuilt source shot. Grading and stabilization do not restore missing geography or factual accuracy.
